...
bool movePiece(int fromRow, int fromCol, int toRow, int toCol, char player) {
    ...
    if (canCapture(player)) {
        if (!king && absDr == 2 && absDc == 2) {
            ...
            while (r != toRow && c != toCol) {
                ...
                if (isOpponent(board[r][c], player) && !enemigoEncontrado) {
                    oppRow = r; oppCol = c;
                    enemigoEncontrado = true;
                    r += stepR; c += stepC;
                    while (r != toRow && c != toCol) {
        ...
        std::cout << "Captura obligatoria activa. Pulsa tecla para seguir.\n";
        (void)_getch();
        return false;
    }
    ...
    if (king && absDr >= 1 && diagonalClear(fromRow, fromCol, toRow, toCol)) {
        board[toRow][toCol] = piece;
        board[fromRow][fromCol] = '.';
        return true;
    }
...
void jugarDamas() {
    ...
    while (true) {
        printBoard();
        std::cout << "turno de " << player << " (formato: fila_origen col_origen fila_destino col_destino): ";
        int fromRow, fromCol, toRow, toCol;
        std::cin >> fromRow >> fromCol >> toRow >> toCol;

        if (!std::cin) {
            std::cout << "Dato incorrecto. Finalizando.\n";
            (void)_getch();
            break;
        }

        if (!movePiece(fromRow, fromCol, toRow, toCol, player)) {
            std::cout << "Movimiento no permitido. Reintenta.\n";
            (void)_getch();
            continue;
        }
        ...
        if (isGameOver(winner)) {
            printBoard();
            std::cout << "Partida concluida. Gana " << winner << ".\n";
            (void)_getch();
            break;
        }
